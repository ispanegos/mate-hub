...
begin
  ...
  if top_messages = p_user_id then badges := badges || 'chiacchierone'; end if;
  if top_expenses = p_user_id then badges := badges || 'banchiere'; end if;
  if top_events = p_user_id then badges := badges || 'organizzatore'; end if;
  if top_rated = p_user_id then badges := badges || 'favorito'; end if;
  if top_chaos = p_user_id then badges := badges || 'caos'; end if;
  if top_reliability = p_user_id then badges := badges || 'roccia'; end if;
